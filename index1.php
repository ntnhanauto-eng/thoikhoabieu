...
<head>
    ...
    <style>
        ...
        .tab-btn.active[data-target="2"] { background: #007bff; color: white; box-shadow: 0 4px 6px rgba(0,123,255,0.2); }
        ...
        /* Màu nền đặc trưng khu vực tiêu đề 2 bé */
        .kid-1-title { background: #f4faf5 !important; border: 2px solid #28a745; }
        .kid-2-title { background: #f3f8ff !important; border: 2px solid #007bff; }
        ...
        /* Cố định thanh tiêu đề khi cuộn dọc dọc */
        thead th { 
            position: sticky; 
            top: 0; 
            z-index: 10; 
        ...
        th, td { border: 1px solid #e2e8f0; padding: 12px 10px; text-align: center; font-size: 14px; word-wrap: break-word; }
        
        /* KÍCH THƯỚC CÁC CỘT ĐẶC THÙ */
        .col-buoi { width: 75px; white-space: nowrap; }
        .col-tiet { width: 110px; min-width: 110px; white-space: nowrap; font-weight: normal; background: #ffffff; } /* Trả về nền trắng tự nhiên trùng màu các cột thứ */

        /* --- HIGHLIGHT CỘT NGÀY HÔM NAY (Đổi màu tự nhiên) --- */
        .current-day { 
            background-color: #fffbeb !important; /* Màu vàng kem nhạt dịu mắt */
        }
        th.current-day { background-color: #f59e0b !important; color: white !important; }
        ...
        .subject-empty { color: #94a3b8; font-style: italic; font-size: 12px; }
        ...
        @media print {
            ...
            th { background-color: #f2f2f2 !important; color: #000 !important; position: static; }
            .current-day { background-color: transparent !important; }
        }

        /* --- TỐI ƯU ĐIỆN THOẠI MOBILE --- */
...
<body>
    <div class="container">
        ...
        foreach ($kids as $id_be => $info):
            ...
            // Tạo style màu nền đồng bộ ép trực tiếp vào th để tránh bị sticky header đè mất màu gốc
                    ...
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th class="<?php echo ($today_field == 'thu2') ? 'current-day' : ''; ?>" <?php echo ($today_field == 'thu2') ? '' : $th_bg_style; ?>>THỬ 2</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <?php if (!in_array($current_buoi, $displayed_buoi)): ?>
                                    <td rowspan="<?php echo $buoi_counts[$current_buoi]; ?>" style="font-weight:bold; background:#f8f9fa; vertical-align: middle; color: #475569;">
                                        <?php echo htmlspecialchars($current_buoi); ?>
                                    </td>
                                    <?php $displayed_buoi[] = $current_buoi; ?>
                                <?php endif; ?>

                                <td class="col-tiet"><?php echo htmlspecialchars($row['tiet']); ?></td>
                                
                                <td class="<?php echo ($today_field == 'thu2') ? 'current-day' : ''; ?>"><span class="cell-content"><?php echo htmlspecialchars($row['thu2']); ?></span></td>
                                <td class="<?php echo ($today_field == 'thu3') ? 'current-day' : ''; ?>"><span class="cell-content"><?php echo htmlspecialchars($row['thu3']); ?></span></td>
                                <td class="<?php echo ($today_field == 'thu4') ? 'current-day' : ''; ?>"><span class="cell-content"><?php echo htmlspecialchars($row['thu4']); ?></span></td>
                                <td class="<?php echo ($today_field == 'thu5') ? 'current-day' : ''; ?>"><span class="cell-content"><?php echo htmlspecialchars($row['thu5']); ?></span></td>
                                <td class="<?php echo ($today_field == 'thu6') ? 'current-day' : ''; ?>"><span class="cell-content"><?php echo htmlspecialchars($row['thu6']); ?></span></td>
                                <td class="<?php echo ($today_field == 'thu7') ? 'current-day' : ''; ?>"><span class="cell-content"><?php echo htmlspecialchars($row['thu7']); ?></span></td>
                                <td class="<?php echo ($today_field == 'cn') ? 'current-day' : ''; ?>"><span class="cell-content"><?php echo htmlspecialchars($row['cn']); ?></span></td>
                            </tr>
                            <?php 
                                endforeach; 
                            else: 
    ...
    // TỰ ĐỘNG THAY THẾ Ô TRỐNG HOẶC DẤU GẠCH BẰNG TEXT PHÙ HỢP
    ...
    // HÀM IN ẤN TRUY XUẤT ỔN ĐỊNH QUA IFRAME CHẠY NGẦM
